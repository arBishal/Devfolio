  ...
  return (
    <div className="space-y-2 md:space-y-4 text-sm md:text-base">
      <div className="flex gap-3 md:gap-4 items-baseline">
        <span className="text-t-muted text-xs md:text-sm w-16 md:w-24 flex-shrink-0 capitalize">email</span>
      ...
      {links.map((item) => (
        <div key={item.label} className="flex gap-3 md:gap-4 items-baseline">
          <span className="text-t-muted text-xs md:text-sm w-16 md:w-24 flex-shrink-0 capitalize">{item.label}</span>
          <a
            href={item.url}
            target="_blank"
